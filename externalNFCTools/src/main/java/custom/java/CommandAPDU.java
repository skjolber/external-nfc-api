...
public final class CommandAPDU implements java.io.Serializable {
    ...
    public CommandAPDU(int cla, int ins, int p1, int p2, byte[] data,
        ...
        if (dataLength > 65535) {
            throw new IllegalArgumentException("dataLength is too large: " + dataLength);
        }
        if (ne < 0) {
            throw new IllegalArgumentException("ne must not be negative: " + ne);
        }
        if (ne > 65536) {
            throw new IllegalArgumentException("ne is too large: " + ne);
        }
        this.ne = ne;
        this.nc = dataLength;
        if (dataLength == 0) {
