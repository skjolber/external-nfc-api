...
public class ServiceUtil {
    ...
    public static void mifareClassic(Context context, ApduTag acsTag) {
        ...
        if (responseAPDU.isSuccess()) {
            uid = com.github.skjolber.nfc.command.Utils.reverse(responseAPDU.getData());
        } else {
            // try the old way
                ...
                if (historicalBytes.length >= 11) {
                    ...
                    if (standard == 0x11) {
                        ...
                        switch (tagId) {
                            ...
                            default: {
                                Log.w(TAG, "Unknown tag id " + com.github.skjolber.nfc.command.Utils.toHexString(new byte[]{historicalBytes[13], historicalBytes[14]}) + " (" + Integer.toHexString(tagId) + ")");
                            }
                        }
                    } else {
                        switch (tagId) {
                            ...
                            default: {
                                ...
                                if ((historicalBytes[13] & 0xFF) == 0xFF) {
                                    Log.i(TAG, "Assume android device for " + com.github.skjolber.nfc.command.Utils.toHexString(new byte[]{historicalBytes[13], historicalBytes[14]}) + " (" + Integer.toHexString(tagId) + ")");
